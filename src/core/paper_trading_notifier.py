...
def _format_entry_message(pos: PaperTradingPosition, sig: StrategySignalRun | None) -> tuple[str, str]:
    """格式化建仓通知，返回 (title, body)。"""
    name = pos.stock_name or pos.stock_symbol
    title = f"【模拟盘建仓】{name}"

    # 盈亏比
    rr_str = ""
    if pos.stop_loss and pos.target_price and pos.entry_price:
    ...
    score_str = ""
    strategy_str = pos.strategy_code or ""
    if sig:
        if sig.rank_score:
            score_str = f" | 评分: {sig.rank_score:.1f}"
        if sig.strategy_code:
            strategy_str = sig.strategy_code

    body = (
        f"股票: {name} ({pos.stock_symbol}.{pos.stock_market})\n"
        f"方向: 买入\n"
        f"买入价: {pos.entry_price:.2f} | 数量: {pos.quantity} 股\n"
        f"止损价: {pos.stop_loss:.2f} | 目标价: {pos.target_price:.2f}\n"
        f"策略: {strategy_str}{score_str}"
        f"{rr_str}"
    )
    return title, body


def _format_exit_message(pos: PaperTradingPosition, trade: PaperTradingTrade) -> tuple[str, str]:
    """格式化平仓通知，返回 (title, body)。"""
    name = pos.stock_name or pos.stock_symbol
    pnl_sign = "+" if trade.pnl >= 0 else ""
    title = f"【模拟盘平仓】{name} {pnl_sign}{trade.pnl:.2f}"

    reason = EXIT_REASON_LABELS.get(trade.exit_reason, trade.exit_reason)
    body = (
        f"平仓原因: {reason}\n"
        f"买入价: {trade.entry_price:.2f} → 卖出价: {trade.exit_price:.2f}\n"
        f"盈亏: {pnl_sign}{trade.pnl:.2f} ({pnl_sign}{trade.pnl_pct:.2f}%) | 持仓: {trade.holding_days}天"
    )
    return title, body


def _format_premarket_plan(signals: list[StrategySignalRun], account: PaperTradingAccount) -> tuple[str, str]:
    """格式化盘前计划，返回 (title, body)。"""
    title = "【模拟盘盘前计划】"
    if not signals:
    ...
    for i, sig in enumerate(signals[:10], 1):
        ...
        lines.append(f"{i}. {name} ({sig.stock_symbol}.{sig.stock_market}){entry_range}{score_str} [{sig.strategy_code}]")
...
async def notify_entry(pos: PaperTradingPosition, sig: StrategySignalRun | None) -> None:
    """建仓通知（异步，失败仅日志）。"""
    try:
...
async def notify_exit(pos: PaperTradingPosition, trade: PaperTradingTrade) -> None:
    """平仓通知（异步，失败仅日志）。"""
    try:
        if not _is_mode_enabled("pt_notify_realtime"):
            return
...
async def send_premarket_plan() -> None:
    ...
    try:
        ...
        try:
            ...
            signals = query.order_by(StrategySignalRun.rank_score.desc()).limit(20).all()
